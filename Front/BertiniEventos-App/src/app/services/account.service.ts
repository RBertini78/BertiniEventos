...
public register(model: any): Observable<void>{
  return this.http.post<User>(this.baseUrl + 'RegisterUser', model).pipe(take(1),map((response: User) => {
    const user = response;
    if(user){
      //localStorage.setItem('user', JSON.stringify(user));
      this.setCurrentUser(user);
...
public getUser(): Observable<UserUpdate>{
  ...
  return this.http.get<UserUpdate>(this.baseUrl + 'GetUser', { headers }).pipe(take(1));
}

public updateUser(model: UserUpdate): Observable<void>{
  const token = localStorage.getItem('user')? JSON.parse(localStorage.getItem('user')!).token : '';
  const headers = new HttpHeaders().set('Authorization', `Bearer ${token}`);
  return this.http.put<UserUpdate>(this.baseUrl + 'UpdateUser', model, {headers}).pipe(take(1), map((user: UserUpdate) => {
    this.setCurrentUser(user);
    }
   )
  )
